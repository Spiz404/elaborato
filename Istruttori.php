    ...
    if(isset($_POST["del"])){
        if(isset($_POST['vet-del']) && !empty($_POST['vet-del'])){
            ...
            for($i = 0; $i < count($vet); $i++){
                $q = "DELETE FROM Istruttori WHERE Username =\"$vet[$i]\" ";
    ...
    if(isset($_POST["mod"])){
        if(isset($_POST['modifica']) && !empty($_POST['modifica'])){ // controllo se non è vuoto il vettore contenete l'username dell'utente da modificare
            ...
            for($i = 0; $i < count($utenti); $i++){
                $utente = $utenti[$i];
                $q = "SELECT * FROM Istruttori WHERE Username = '$utente'"; // selezione dati utente da modificare
        ...
        $email = $_POST["email"];
        $q = "UPDATE Istruttori SET Username = '$username', Nome = '$nome', Cognome = '$cognome', Email = '$email' WHERE Username = '$user'";
        $ris = $con -> query($q) or die(mysqli_error($con));
    }
