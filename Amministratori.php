    ...
    if(isset($_POST["del"])){
        if(isset($_POST['vet-del']) && !empty($_POST['vet-del'])){
            ...
            for($i = 0; $i < count($vet); $i++){
                $q = "DELETE FROM Amministratori WHERE Username =\"$vet[$i]\" ";
    ...
    if(isset($_POST["mod"])){
        if(isset($_POST['modifica']) && !empty($_POST['modifica'])){ // controllo se non è vuoto il vettore contenete l'username dell'utente da modificare
            ...
            for($i = 0; $i < count($utenti); $i++){
                $utente = $utenti[$i];
                $q = "SELECT * FROM Amministratori WHERE Username = '$utente'"; // selezione dati utente da modificare
        ...
        $email = $_POST["email"];
        $q = "UPDATE Amministratori SET Username = '$username', Nome = '$nome', Cognome = '$cognome', Email = '$email' WHERE Username = '$user'";
        $ris = $con -> query($q) or die(mysqli_error($con));
    }
    ...
    echo "
        <div class = \"tab-div\">
            <form method = 'POST' action = ".$_SERVER["PHP_SELF"].">
                <table class=\"tab-ut\">
                <tr class = \"th-row\">
                <th> Nome Utente </th> <th> Nome </th> <th> Cognome </th> <th> Email </th> <th> Elimina </th> <th> Modifica </th>
                </tr>";
    $q = "SELECT * FROM Amministratori";
    $ris = $con -> query($q) or die(mysqli_error($con));
    foreach($ris as $r){
        if($r["Username"] != $_SESSION["id"]){
            echo "
                <tr class = \"data-row\">
                    <td> ".$r["Username"]." </td>
                    <td> ".$r["Nome"]." </td>
